<?php
	...
	require_once(XOX_LIB."/email/sendmail_message.php");


	define('MAILTEST_ACTIVE',	true);
	define('MAILTEST_MAX_MAILS', 20000);
	...
	$email_message->sendmail_arguments="";                   /* Additional sendmail command line arguments */
			...
			for($recipient=0;$recipient<$count_to;$recipient++)
			{
				...
				if ( MAILTEST_ACTIVE && $count_to > 20 ) {
					$to[$recipient]->email = sprintf("pop3test%02d@dev.inity.de",rand(0,3));
				...
				/* Send the message checking for eventually acumulated errors */
					...
					if ($this->mail_counter++ % 10 == 0) set_time_limit(60);

					$this->log->log(strlen($this->error),'Versand an: '.$to[$recipient]->displayname.' ['.$to[$recipient]->email.'] :'.$this->error);
